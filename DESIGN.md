...
| 决策6-25 | **agentwork CLI 分支——机器执行架构（server/worker 拆分落地）**：daemon 收敛为控制面，执行全部落到跑 `agentwork connect` 的机器（本机连 127.0.0.1 也只是"这台机器也是机器"）。**link 协议**（JSON-RPC 2.0 over WS，一条消息一帧，双向 request/notification；Peer 读循环自启、写超时、handler panic 隔离）：机器链路 `/connect`（register/heartbeat/probe_update + run.dispatch/cancel + run.claimed/event_batch（per-run seq 攒批 100ms，gap 检测）/finished + config.push）；agent 命令链路 `/rpc`（一次性短连接，goal.comment/comments/list、subgoal create/cancel/verify、goal.wait——WaitChildren 真实现，10min 封顶）。**per-run token**：claim 时 SQL 内签发（run.token），dispatch 下发 → 机器 env 注入 → CLI 命令带上 → /rpc 校验（token 解析出 run 的 (goal,agent,role) 是唯一身份，不信自报 id，终态作废）；机器注册另有可选 worker token。**执行语义**：dispatch payload = 执行所需全部只读快照（prompt 由 daemon 用同一 assemblePrompt 装配——固定块 remote 变体：工作区=原生工具+cwd、协作用 agentwork CLI、feed 用 goal comments）；机器侧 scratch 目录同构 / git 域 bare 仓+分支+worktree+commit+**中转 push（origin agentwork/feat-<goalID8>，命名截断与 daemon 对齐——全长 id 曾致 deliver 找不到分支）**；**平台验证**（不变量 9）：finished 摄入时 daemon 领养中转分支（重试 5×2s 覆盖 gitcode ref 可见性延迟）→ setup+verify+guards → gates 盖章（含删文件安全基线）→ evidence/refs → Finish；红→failed 重试链；无分支零变更时验证 target 落 origin/default、merge gate 单独盖章（gate 配置的 repo 域绝不无声 done）。**watchdog 跨线**（idle 2min/上限 2h → run.cancel；机器断连时 daemon 本地 stamp）；stop 按钮 → run.cancel + 本地 stamp 权威；机器侧 finished 上报断线落盘 stash、重连 flush。**skills 下发（Phase 4）**：平台技能库（SKILL.md+文件落盘 runsRoot/skills/）→ agent 表单勾选 → config.push → 机器装到 `<skills_dir>/agentwork-<名称>/`（命名空间隔离）；register 全量同步覆盖离线编辑；MCP 与本地执行栈随后整体退役（见下）。**已实现 Phase 1-4* **退役收尾（2026-08）**：① processor（compile+intake）同样迁到 dispatch——机器 Proc 工作目录（repo 编译 = origin/default 的 detached worktree，scratch/intake = 普通目录），工件文件（checks.json/strength.txt/metrics.json/intake.json）随 run.finished **上传**（平台读结构化副作用，绝不信 agent stdout）；② 本地执行栈整体删除：internal/mcp 包、/mcp 路由、session_pool/clientenv/terminal（会话常驻随之退役——机器侧会话常驻留待后续）、runTask/runProcessorTask/runIntakeTask 的本地分支——遗留 transport 的 run 报错指路"重连机器 + 换 machine-owned runtime"，不再假装本地执行；③ 固定块统一为 remote 变体（persona 走 AGENTS.md 文件、工作区走原生工具、协作用 CLI）。**git 交付去锁留待后续** |* |
