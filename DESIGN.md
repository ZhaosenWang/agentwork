...
| 决策6-18 | **平台文本语言规则**：平台固定文本一律英文（对 agent 的 prompt/指引、系统通知评论、wake note）——**语言由物料承载**：agent 的评论/汇报经 LANGUAGE 指令跟随 goal 语言，卡点 reason、审查意见等人类面内容本身就是物料原文。唯一例外：sub-goal 派发评论——平台模板包裹的是 leader 写的工作项（物料），模板语言跟随物料（subgoal 内部私有 CJK 检测，仅此一处，无全局语言模型）。**派发评论精简**：只写标题 + 一句摘要——描述全文已在 Task 段注入，feed 不必重复全文——**已实现** |
