...
| 决策6-17 | **唤醒原因随 run 快照（wake note）**：owner spawn 时，在推导 attention 的**同一事务**里把唤醒原因编译成文本写进 `run.wake_note`（recovery→"子任务「X」失败——cancel 或重建"；integration→"N 个变更待集成"；wrapup→"子任务「X」已验证、未产生变更——交付物在评论区，请复核收尾"；多类并存逐条列出）。prompt 从 **run 行**读——`goal.attention` 是活值（每次 reconcile 重推导；wrapup 谓词在 owner 入队后自失效，随后的 `sub_goal.verified` 等事件再触发 reconcile 会把"为什么唤醒你"在 prompt 构建前擦掉——实测 owner 只能靠 feed 末尾碰运气推断，契约 "Woken with an Owner Attention section?" 被打破），留给 UI badge/notify 信号。note 是英文平台文本（决策 6-18），子任务标题按物料原文嵌入。attention 活值段保留为兜底（无 note 的旧行/其它路径）——**已实现** |
