...
// ACP-style approval buttons (Allow Once / Allow Always / Deny). The
// approvalID is embedded in every button's value so the card action callback
// can correlate clicks. "allow_always" remembers the decision for the session
// (same tool+args won't ask again).
...
// decision. The card body is a collapsible_panel that starts collapsed — the
// panel header shows the decision summary, and the hidden body contains the
// tool name, args, and optional reason. The user can expand to see details.
// The header colour reflects the outcome.
